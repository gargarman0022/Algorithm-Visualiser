...
/* Adding weight to an edge. Using some trignometry to find out a point at a distance of p from the midpoint of points i and j and perpendicular to line joining i and j */
  ...
  async function animate() {
    for (let i = 0; i < noOfDataPoints; i++) {
      ...
      for (let j = 0; j < noOfDataPoints; j++) {
        if (!relaxed[j] && distance[j] < tempMin) {
          tempMin = distance[j];
          nextNode = j;
        }
      }
      if (nextNode !== -1) {
        relaxed[nextNode] = true;
        modifyNode(nextNode);
        await sleep(1000);
        for (let i = 0, l = adj[nextNode].length; i < l; i++) {
          let [neigh, cost] = adj[nextNode][i];
          if (relaxed[neigh]) continue;
          await animateLine(nextNode, neigh);
          await sleep(500);
          if (distance[neigh] > distance[nextNode] + cost) {
            distance[neigh] = distance[nextNode] + cost;
            data[neigh].innerText = distance[neigh];
          }
        }
      } else {
        return;
...
upperLayer.addEventListener("click", function (e) {
  ...
  element.innerText = `Inf`; /* setting initial value of the distance of node as inf */
